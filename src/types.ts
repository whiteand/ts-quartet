...
export type ConstantSchema = undefined | null | boolean | number | string | symbol
export type Schema =
  | FunctionSchema
  | ConstantSchema
  | IObjectSchema
...
type IfAny<T, Any, NotAny> = true extends T
  ? ('1' extends T
      ? (1 extends T
          ? ({} extends T
              ? ((() => void) extends T ? (null extends T ? (Any) : (NotAny)) : NotAny)
              : NotAny)
          : NotAny)
      : NotAny)
  : NotAny
...
export type ErrorBoundary = (explanations: any[], params: IErrorBoundaryParams) => any

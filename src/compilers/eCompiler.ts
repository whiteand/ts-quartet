...
function explanation(value: any, path: KeyType[], schema: TSchema): IExplanation {
  return {
    value,
...
function validate(
  ...
  switch (schema.type) {
    ...
    case SchemaType.Function:
      ...
      return false
    case SchemaType.Max:
      if (schema.isExclusive ? value < schema.maxValue : value <= schema.maxValue) {
        return true
      }
      explanations.push(explanation(value, path, schema))
      return false
    case SchemaType.MaxLength:
      ...
      return false
    case SchemaType.Min:
      if (schema.isExclusive ? value > schema.minValue : value >= schema.minValue) {
        return true
      }
      explanations.push(explanation(value, path, schema))
      return false
    ...
    case SchemaType.Object:
      ...
      for (let i = 0; i < props.length; i++) {
        ...
        path.push(prop)
        if (!validate(propValue, propsSchemas[prop as string], path, explanations)) {
          path.pop()
          return false
        }
        path.pop()
...
export function eCompiler<T = any>(rawSchema: RawSchema): CompilationResult<T, any> {

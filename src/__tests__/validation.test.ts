...
test('function schema', () => {
  const v = quartet()
  let lastArgs: { value: any, explanations?: any[], parents?: IKeyParentSchema[], settings?: InstanceSettings } | null = null
  ...
  expect((v({
    evenNumber: isEven
  })(parent2))).toEqual(true)
  expect(lastArgs).toEqual({
    explanations: [],
    parents: [{ key: 'evenNumber', parent: parent2, schema: { evenNumber: isEven } }],
    value: 2
  })
  const parent3 = { evenNumber: 3 }
  expect((v({
    evenNumber: isEven
  })(parent3, [1]))).toEqual(false)
  expect(lastArgs).toEqual({
    explanations: [1],
    parents: [{ key: 'evenNumber', parent: parent3, schema: { evenNumber: isEven } }],
    value: 3
  })

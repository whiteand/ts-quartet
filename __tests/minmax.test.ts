import { v } from "../index";
import { puretables, snapshot } from "./utils";

describe("min", () => {
  test("exclusive", () => {
    const validator = v(v.min(5, true));
    puretables(validator, [6, 7, 8, 9, 5.1, Infinity], [1, 2, 3, 4, 5, -Infinity]);
    snapshot(validator);
  });
  test("inclusive", () => {
    const validator = v(v.min(5));
    puretables(validator, [5, 6, 7, 8, 9, 5.1, Infinity], [1, 2, 3, 4, -Infinity]);
    snapshot(validator);
  });
});
...
describe("max", () => {
  test("exclusive", () => {
    ...
    puretables(validator, [1, 2, 3, 4, -Infinity], [5, 6, 7, 8, 9, 5.1, Infinity]);
    snapshot(validator);
  });
  test("inclusive", () => {
